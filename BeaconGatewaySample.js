    ...
    Bleacon.on('discover', function (bleacon) {
        ...
        if (beacons[beaconId] === undefined) {
            if (calculateDistance(bleacon.rssi, bleacon.measuredPower) <= MAX_DISTANCE) {
                ...
                var metadata = {};
                metadata[iotcs.device.GatewayDevice.DeviceMetadata.MANUFACTURER] = 'Estimote';
                metadata[iotcs.device.GatewayDevice.DeviceMetadata.DEVICE_CLASS] = 'LE';
                metadata[iotcs.device.GatewayDevice.DeviceMetadata.PROTOCOL] = 'Bluetooth-LE';
                metadata[iotcs.device.GatewayDevice.DeviceMetadata.PROTOCOL_DEVICE_CLASS] = 'iBeacon';
                metadata[iotcs.device.GatewayDevice.DeviceMetadata.MODEL_NUMBER] = 'EST';
                metadata[iotcs.device.GatewayDevice.DeviceMetadata.SERIAL_NUMBER] = major + ':' + minor;
                metadata["UUID"] = bleacon.uuid;
        ...
        if (beacon.type === 'uid') {
            ...
            if (beacons[beaconId] === undefined) {
                ...
                if (beacon.txPower !== undefined && calculateDistance(beacon.rssi, beacon.txPower - 41) <= MAX_DISTANCE) {
                    ...
                    metadata[iotcs.device.GatewayDevice.DeviceMetadata.DEVICE_CLASS] = 'LE';
                    metadata[iotcs.device.GatewayDevice.DeviceMetadata.PROTOCOL] = 'Bluetooth-LE';
                    metadata[iotcs.device.GatewayDevice.DeviceMetadata.PROTOCOL_DEVICE_CLASS] = 'Eddystone';
                    metadata[iotcs.device.GatewayDevice.DeviceMetadata.MODEL_NUMBER] = 'EST';
                    metadata[iotcs.device.GatewayDevice.DeviceMetadata.SERIAL_NUMBER] = beacon.instance;
                    metadata["UUID"] = beaconId;
...
function simulateBeacons(device) {
    ...
    setInterval(updateSimulatedBeacons, 5000);
}

function registerSimulatedBeacon(device, minor) {
    var major = MAJOR_NUM;
    var iBeacon = (minor % 2 === 0);
    var model, modelUrn;
    var minorStr = ('0000' + minor.toString(16)).substr(-4);
    var beaconId = gateway.getEndpointId() + '_Sample:' + major + ':' + minorStr;
    var metadata = {};
    metadata[iotcs.device.GatewayDevice.DeviceMetadata.MANUFACTURER] = 'Sample';
    metadata[iotcs.device.GatewayDevice.DeviceMetadata.DEVICE_CLASS] = 'LE';
    metadata[iotcs.device.GatewayDevice.DeviceMetadata.PROTOCOL] = 'Bluetooth-LE';
    metadata[iotcs.device.GatewayDevice.DeviceMetadata.SERIAL_NUMBER] = major + ':' + minorStr;
    metadata["UUID"] = gateway.getEndpointId();
    metadata["major"] = major;
    metadata["minor"] = minorStr;

    if (iBeacon) {
        metadata[iotcs.device.GatewayDevice.DeviceMetadata.PROTOCOL_DEVICE_CLASS] = 'iBeacon';
        metadata[iotcs.device.GatewayDevice.DeviceMetadata.MODEL_NUMBER] = 'Sample iBeacon';
        model = ibeaconModel;
        modelUrn = ibeaconModelUrn;
    }
    else {
        metadata[iotcs.device.GatewayDevice.DeviceMetadata.PROTOCOL_DEVICE_CLASS] = 'Eddystone';
        metadata[iotcs.device.GatewayDevice.DeviceMetadata.MODEL_NUMBER] = 'Sample Eddystone';
