...
class HelloWorldSample {
    constructor() {
        ...
        if (dcd.isActivated()) {
            ...
            dcd.activate(['urn:com:CSG:CSL:RFIDReader'], device => {
                dcd = device;
    ...
    getDeviceModel(device) {
        ...
        device.getDeviceModel('urn:com:CSG:CSL:RFIDReader', response => {
    ...
    startVirtualDevice(device, id, deviceModel) {
        ...
        virtualDevice.update({"ora_rssi": 5});
        virtualDevice.close();
...
iotcs.oracle.iot.tam.store = (process.argv[2]);
iotcs.oracle.iot.tam.storePassword = (process.argv[3]);
